...
for (j in 1:length(model_name)) {
  ...
  tictoc::tic()
  fits <- do_jitter(fit, n = 10, amount = 0.5, seed = 23, cores = 5, use_fitted = FALSE, return_models = TRUE, do_sd = TRUE)
  tictoc::toc()

  saveRDS(fits, file = file.path("jitter", paste0("jitter_model_", i)))
...
if (FALSE) {
  ...
  fits <- readRDS(file = file.path("jitter", paste0("jitter_", i, ".rds")))
  ...
  g <- like %>%
    ...
    left_join(names_df) %>%
    filter(!is.na(variable2), !variable %in% c("loglike_Cinit_mfr", "conv", "fn")) %>%
