...
len_cat <- data.frame(
  Name = c("US_RR_66_114", "US_RR_115_144", "VAST_US_RR_177", "US_RR_145", "US_RR_195", "US_RR_66_144"),
...
cpue <- readxl::read_excel(xlsx_file, sheet = "CPUE") %>%
  ...
  mutate(CV = ifelse(Name == "VAST_US_RR_177", CV/Index, CV))

cpue_use <- c("SPN_BB", "SPN_FR_BB", "MOR_SPN_TRAP", "sdmTMB_POR-MOR_TRAP", "JPN_LL_Eatl_Med",
              "JPN_LL_NEAtl1", "JPN_LL_NEAtl2", "US_RR_66_144", "VAST_US_RR_177", "US_RR_145",
              "US_RR_195", "MEXUS_GOM_PLL", "JPN_LL_West1", "VAST_JPN_LL_West2", "JPLL_GOM",
              "VAST_CAN_SWNS_RR")
...
index_names_matrix <- rbind(
  index_names,
  index_names %>% filter(grepl("VAST CAN ACO SUV", Name)) %>% mutate(Stock = 1)
) %>%
...
#### Save objects
dir_save <- "model_input/06.30.2026_VAST"
